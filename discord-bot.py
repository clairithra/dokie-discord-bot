...
FAQ_RESPONSES = {
    ...
    "not_sure": "I'm not quite sure about that one! For the best help, please email **support@dokie.ai** with your question and our team will get back to you within 24 hours. 💙"
}

# Intent classification prompt
CLASSIFICATION_PROMPT = """You are a support bot classifier. Classify the user's question into ONE of these categories:

- download_app: User asking where to download Dokie application/software
- export_project: User asking how to download/export/save their presentation/project
- sign_in: Login or authentication issues
- crash: App crashed, broken, not working, errors, bugs
- refund: Refund requests, subscription cancellation, billing issues
- export_issues: Exported file looks wrong, formatting issues, missing animations
- generation_error: AI generation failed, won't generate, stuck generating
- generation_bad: AI generated poor quality content, wrong content
- slow: Performance issues, laggy, slow loading
- version_history: How to find previous versions, undo changes
- mobile: Mobile app questions
- credits: Questions about credit balance, running out of credits
- feature_request: Suggestions for new features
- not_sure: Question doesn't fit any category above
...
async def classify_intent(question: str) -> str:
    """Use DeepSeek to classify user intent"""
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                return category if category in FAQ_RESPONSES else "not_sure"
            else:
...
async def on_message(message):
    ...
    # Skip if message doesn't look like a question
    if len(message.content) < 10:
        return

    # Classify intent
    category = await classify_intent(message.content)

    # Send response
    if category in FAQ_RESPONSES:
